...
function computeinsidecuda(unpairedlogprobs::Array{Float32,1}, pairedlogprobs::Array{Float32,2}, grammar::KH99, B::Float64=1.0, zonly::Bool=true)
  ...
  if zonly
    ...
    global d_insideL
    global d_insideF
    if len != cachedlen && length(d_insideS) == len*len  && length(d_insideL) == len*len  && length(d_insideF) == len*len
   ...
   # d_insideS = convert(CuPtr{Cfloat}, CUDAdrv.Mem.alloc(CUDAdrv.Mem.DeviceBuffer, len*len*4))
    ...
    #CuArrays.unsafe_free!(d_insideS)
    #CuArrays.unsafe_free!(d_insideL)
    #CuArrays.unsafe_free!(d_insideF)
