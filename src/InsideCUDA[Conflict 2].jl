...
function computeinsidecuda(unpairedlogprobs::Array{Float32,1}, pairedlogprobs::Array{Float32,2}, grammar::KH99, B::Float64=1.0, zonly::Bool=true)
  ...
  #d_pairedlogprobs = CUDAdrv.Mem.alloc(Mem.Device, len*len*4)

  if zonly
    d_insideS = CuArray(zeros(Float32, len*len))
    d_insideL = CuArray(zeros(Float32, len*len))
    d_insideF = CuArray(zeros(Float32, len*len))
   # d_insideS = convert(CuPtr{Cfloat}, CUDAdrv.Mem.alloc(CUDAdrv.Mem.DeviceBuffer, len*len*4))
    #d_insideL = convert(CuPtr{Cfloat}, CUDAdrv.Mem.alloc(CUDAdrv.Mem.DeviceBuffer, len*len*4))
    #d_insideF = convert(CuPtr{Cfloat}, CUDAdrv.Mem.alloc(CUDAdrv.Mem.DeviceBuffer, len*len*4))
    #d_insideS = CUDAdrv.Mem.alloc(Mem.Device, len*len*4)
    #d_insideL = CUDAdrv.Mem.alloc(Mem.Device, len*len*4)
    #d_insideF = CUDAdrv.Mem.alloc(Mem.Device, len*len*4)
    
    cudacall(cudainitialiseinside2, Tuple{CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat}, Cint}, d_insideS, d_insideL, d_insideF, d_unpairedlogprobs, len, blocks=div(len+blocksize-1, blocksize), threads=blocksize)
    for a=1:len-1
      cudacall(cudainside2, Tuple{CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat}, Cint, Cint, Cfloat}, d_insideS, d_insideL, d_insideF, d_pairedlogprobs, d_unpairedlogprobs, a, len, Float32(B), blocks=div(len+blocksize-1, blocksize), threads=blocksize)
    end
    cudacall(cudainsidez, Tuple{CuPtr{Cfloat}, CuPtr{Cfloat}, Cint}, d_insideS, d_Z, len, blocks=div(len+blocksize-1, blocksize), threads=blocksize)
    #=
    ...
    #=
    if pointer(d_insideS.buf) != CU_NULL
        @apicall(:cuMemFree, (CuPtr{Cvoid},), d_insideS.buf)
    end
    if pointer(d_insideL.buf) != CU_NULL
        @apicall(:cuMemFree, (CuPtr{Cvoid},), d_insideL.buf)
    end
     if pointer(d_insideF.buf) != CU_NULL
        @apicall(:cuMemFree, (CuPtr{Cvoid},), d_insideF.buf)
    end=#

    #CuArrays.unsafe_free!(d_unpairedlogprobs)
    #CuArrays.unsafe_free!(d_pairedlogprobs)
    #my_unsafe_free!(d_insideS)
    #my_unsafe_free!(d_insideL)
    #my_unsafe_free!(d_insideF)
    #CuArrays.unsafe_free!(d_Z)

    finalize(d_unpairedlogprobs)
    finalize(d_pairedlogprobs)
    #CUDAdrv.Mem.free(d_pairedlogprobs)

    finalize(d_insideS)
    finalize(d_insideL)
    finalize(d_insideF)
    #CUDAdrv.Mem.free(d_insideS)
    #CUDAdrv.Mem.free(d_insideL)
    #CUDAdrv.Mem.free(d_insideF)
    finalize(d_Z)
    
    destroy!(ctx)
    synchronize(ctx)
    #CUDAnative.device_reset!(dev)
    CuArrays.reclaim()
    return Z
  else
    d_insideS = CuArray(zeros(Float32, len*len))
