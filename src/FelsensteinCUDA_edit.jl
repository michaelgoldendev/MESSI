...
function felsensteincuda(dataset::Dataset, params::ModelParameters, museparams::Array{MuseSpecificParameters,1}, maxbasepairdistance::Int=1000000, keepmuseconditionals::Bool=false)
    ...
    for node in nodelist
        ...
        push!(storelist, CuArray(zeros(Float32, datalen*(alphabet+1))))
    end
    ...
    for siteCat1=1:params.siteCats
        for siteCat2=1:params.siteCats
            for musespecificparams in museparams
                ...
                while length(stack) > 0
                    ...
                    if isleafnode(node)
                        ...
                        completed[completednode] = completednode
                            ...
                            cudacall(cudafelsensteinleaves, (Int32, Int32, CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat}, CuPtr{Cfloat},), Int32(dataset.numcols), Int32(datalen), d_x, d_y, d_data, storelist[nodeindex], blocks=div(datalen+blocksize-1, blocksize), threads=blocksize)
                ...
                cudacall(cudasumfinal, (Int32,Int32,CuPtr{Cfloat},CuPtr{Cfloat}, CuPtr{Cfloat}), alphabet, Int32(datalen), d_freqs, storelist[1], d_logliks, blocks=div(datalen+blocksize-1, blocksize), threads=blocksize)
                if keepmuseconditionals
